...
<div class="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
    <div>
        <h1 class="text-2xl font-bold text-gray-900 border-l-4 border-blue-600 pl-3">Configuración de Facturación Electrónica</h1>
    ...
    <div class="p-6">
        <form id="configForm" class="space-y-6" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="razon_social" value="<?= htmlspecialchars($config['razon_social']) ?>" required
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Nombre Comercial</label>
                        <input type="text" name="nombre_comercial" value="<?= htmlspecialchars($config['nombre_comercial'] ?? '') ?>"
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Dirección Completa</label>
                        <input type="text" name="direccion" value="<?= htmlspecialchars($config['direccion']) ?>" required
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="distrito" value="<?= htmlspecialchars($config['distrito']) ?>" required
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Provincia</label>
                            <input type="text" name="provincia" value="<?= htmlspecialchars($config['provincia']) ?>" required
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Departamento</label>
                            <input type="text" name="departamento" value="<?= htmlspecialchars($config['departamento']) ?>" required
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                        </div>
                    </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="sol_usuario" value="<?= htmlspecialchars($config['sol_usuario']) ?>" required
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Clave SOL</label>
                            <input type="password" name="sol_clave" value="<?= htmlspecialchars($config['sol_clave']) ?>" required
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 gap-4">
                        <div>
                            ...
                            <input type="password" name="certificado_clave" value="<?= htmlspecialchars($config['certificado_clave']) ?>"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                        </div>
                    </div>
                    ...
                    <div class="mt-4 p-4 border border-gray-200 bg-gray-50 rounded-lg">
                        <label class="block text-sm font-medium text-gray-900">Actualizar Logo de la Empresa (.jpg, .png)</label>
                        <?php if(!empty($config['logo_path'])): ?>
                            <p class="text-xs text-gray-600 mb-2">Logo actual registrado. Seleccionar otro para reemplazar.</p>
                        <?php endif; ?>
                    ...
                    <div class="flex items-center mt-6">
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="estado_facturacion" value="1" class="sr-only peer" <?= $config['estado_facturacion'] ? 'checked' : '' ?>>
                            <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-green-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-green-600"></div>
                            <span class="ml-3 text-sm font-semibold text-gray-700">Produccion Facturacion Electronica</span>
                        </label>
                    </div>
                </div>
            </div>
            ...
            <div class="pt-5 border-t border-gray-200 flex justify-end">
                <button type="submit" id="btnGuardar" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
